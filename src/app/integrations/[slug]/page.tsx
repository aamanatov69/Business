...
export default async function IntegrationPage({ params }: Props) {
  ...
  return <main className="shell section solution-page"><Breadcrumbs items={[{ name: "Главная", path: "/" }, { name: "Интеграции", path: "/integrations" }, { name: page.title, path: `/integrations/${slug}` }]} />
    <JsonLd data={{ "@context": "https://schema.org", "@type": "Service", name: page.title, description: page.description, url: `${SITE_URL}/integrations/${slug}`, areaServed: "KG", provider: { "@type": "Organization", name: SITE_NAME, url: SITE_URL } }} />
